...
export const ingestDocument = async (text: string, documentId: string, metadata: any) => {
    try {
        ...
        await publishToQueue('embedding-results-queue', { documentId, status: 'completed' })
    } catch (error) {
        console.error("ERROR: Failed to ingest document", error)
        await publishToQueue('embedding-results-queue', { documentId, status: 'failed' })
    }
